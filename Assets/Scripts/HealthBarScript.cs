...
public class HealthBarScript : MonoBehaviour {
    ...
    private void HealthChanged(float health, float currentHealth) //
    {
       // Debug.Log("The current health is" + currentHealth); Trying to see if it finds that the health is changed.
        hpTransform.localScale = new Vector3(currentHealth / health, 1f, 1f); // setting the currentHealth to the green bar and the red bar to the remaining health of the enemy. If the positions were swapped (health / currentHealth) the green bar would increase after taking damage
    }
